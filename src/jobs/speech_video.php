...
$name = 'eSpeak/FFMPEG';
$type = 'Speech Rendering to Video';
$user = $argv[1];
$input1 = $argv[2];
$input2 = $argv[3];
$input3 = $argv[4];
...
exec('espeak -m -f '.getenv('LANGUAGE_WORKING').'/'.$input2.'.ssml -w '.getenv('LANGUAGE_WORKING').'/'.$media[1].'.wav');
...
if (!file_exists(getenv('LANGUAGE_RENDERS').'/'.$input1.'/'.$media[1])) {
	...
	while ($number_found == 0) {
		$render_number++;
		if (!file_exists(getenv('LANGUAGE_RENDERS').'/'.$input1.'/'.$render_number.'_'.$media[1])) {
			$number_found = 1;
		}
	}
